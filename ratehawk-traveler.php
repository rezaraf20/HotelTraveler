...
class RH_Hotel_Sync {
    ...
    private function create_room($hotel_post_id, $room_group) {
        ...
        // CRITICAL: Simulate a real admin save to trigger all Traveler hooks
        
        // Set current user temporarily if needed
        $original_user = get_current_user_id();
        if (!$original_user) {
            wp_set_current_user(1);
        }
        
        // Update post to trigger hooks
        wp_update_post([
            'ID' => $room_post_id,
            'post_modified' => current_time('mysql'),
            'post_modified_gmt' => current_time('mysql', 1)
        ...
        wp_cache_delete($room_post_id, 'posts');
        wp_cache_delete($room_post_id, 'post_meta');
        
        // Trigger save_post actions that Traveler hooks into
        ...
        do_action('wp_insert_post', $room_post_id, get_post($room_post_id), false);
        
        // Traveler-specific actions (if they exist)
        do_action('st_after_save_room', $room_post_id);
        do_action('traveler_save_room', $room_post_id);
        ...
        rh_log('Room created and triggered', [
            'room_post_id' => $room_post_id,
            'room_name' => $room_name,
            'hotel_id' => $hotel_post_id
        ], 'info');
    ...
    private function save_room_meta($room_post_id, $hotel_post_id, $room_group) {
        ...
        // CRITICAL: Traveler-specific meta
        ...
        // CRITICAL: Generate calendar data for next 12 months
     ...
     * 
     * Note: This creates a placeholder calendar with availability but price=0
     * This is necessary for Traveler theme to display the room.
     * Real prices will be fetched dynamically from Ratehawk API when user searches.
     */
    private function generate_room_calendar($room_post_id) {
        ...
        for ($timestamp = $start_date; $timestamp <= $end_date; $timestamp = strtotime('+1 day', $timestamp)) {
            ...
            // Each date entry - placeholder with price=0
            // Real prices will come from /search/hp/ API when user searches
            $calendar_data[$date_key] = [
                'status' => 'available',
                'price' => 0, // Placeholder - real price from API
                'number' => 1,
                'adult' => 2,
                'children' => 2
            ];
